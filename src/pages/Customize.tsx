      ...
      <section className="pb-20">
        <div className="container mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-10 items-start">
            {/* Cart Display */}
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative lg:sticky lg:top-28">
              <div className="relative rounded-3xl border border-border/20 bg-card/30 backdrop-blur-sm overflow-hidden">
                <div className="absolute inset-0 opacity-10 blur-[100px] transition-colors duration-700" style={{ backgroundColor: selectedColor.hex }} />
                ...
                <div className="pt-14 pb-8 px-8">
                  <AnimatePresence mode="wait">
                    <motion.div key={selectedColor.id} initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.92 }} transition={{ duration: 0.4 }}>
                      <img src={selectedColor.image} alt={`Zebra Cart in ${selectedColor.label}`} className="w-full object-contain max-h-[420px] drop-shadow-[0_25px_50px_rgba(0,0,0,0.5)] mx-auto" />
                    </motion.div>
                  </AnimatePresence>
                </div>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} className="space-y-6">
                  ...
                  {activeTab === "model" && (
                    <div className="space-y-3">
                      ...
                      {configOptions.map((config) => (
                        <button key={config.id} onClick={() => setSelectedConfig(config)}
                          className={`w-full text-left p-5 rounded-2xl transition-all duration-300 border ${
                            selectedConfig.id === config.id ? "bg-primary/10 border-primary/40 shadow-[0_0_30px_-10px_hsl(var(--primary)/0.15)]" : "bg-card/30 border-border/20 hover:border-border/50"
                          }`}>
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              {selectedConfig.id === config.id && (
                                <div className="w-6 h-6 rounded-full bg-primary flex items-center justify-center"><Check className="w-3.5 h-3.5 text-primary-foreground" /></div>
                              )}
                            </div>
                          </div>
                        </button>
                        ...
                        {colors.map((color) => (
                          <button key={color.id} onClick={() => setSelectedColor(color)}
                            className={`group flex items-center gap-4 p-5 rounded-2xl transition-all duration-300 border ${
                              selectedColor.id === color.id ? "bg-primary/10 border-primary/40" : "bg-card/30 border-border/20 hover:border-border/50"
                            }`}>
                            <div className="relative">
                              <div className="w-14 h-14 rounded-full border-2 transition-all duration-300 group-hover:scale-110"
                                style={{ backgroundColor: color.hex, borderColor: selectedColor.id === color.id ? "hsl(var(--primary))" : "hsl(var(--border))", boxShadow: selectedColor.id === color.id ? `0 0 25px ${color.hex}60` : "none" }} />
                              {selectedColor.id === color.id && (
                                <div className="absolute -top-1 -right-1 w-5 h-5 rounded-full bg-primary flex items-center justify-center"><Check className="w-3 h-3 text-primary-foreground" /></div>
                              )}
                            </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        {seatOptions.map((seat) => (
                          <button key={seat.id} onClick={() => setSelectedSeat(seat)}
                            className={`flex items-center gap-4 p-4 rounded-2xl transition-all duration-300 border ${
                              selectedSeat.id === seat.id ? "bg-primary/10 border-primary/40" : "bg-card/30 border-border/20 hover:border-border/50"
                            }`}>
                            <div className="w-10 h-10 rounded-full border-2 shrink-0" style={{ backgroundColor: seat.hex, borderColor: selectedSeat.id === seat.id ? "hsl(var(--primary))" : "hsl(var(--border))" }} />
                  ...
                  {activeTab === "wheels" && (
                    <div className="space-y-6">
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {tireOptions.map((tire) => (
                            <button key={tire.id} onClick={() => setSelectedTire(tire)}
                              className={`p-4 rounded-2xl text-sm font-bold transition-all duration-300 border ${
                                selectedTire.id === tire.id ? "bg-primary/10 border-primary/40 text-foreground" : "bg-card/30 border-border/20 text-muted-foreground hover:border-border/50"
                              }`}>{tire.label}</button>
                          ))}
                        </div>
                      </div>
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {rimOptions.map((rim) => (
                            <button key={rim.id} onClick={() => setSelectedRim(rim)}
                              className={`p-3 rounded-2xl text-xs font-bold transition-all duration-300 border text-center ${
                                selectedRim.id === rim.id ? "bg-primary/10 border-primary/40 text-foreground" : "bg-card/30 border-border/20 text-muted-foreground hover:border-border/50"
                              }`}>{rim.label}</button>
                          ))}
                        </div>
                      </div>
                        ...
                        {accessoryOptions.map((acc) => (
                          <button key={acc.id} onClick={() => toggleAccessory(acc.id)}
                            className={`w-full flex items-center justify-between p-4 rounded-2xl transition-all duration-300 border ${
                              selectedAccessories.includes(acc.id) ? "bg-primary/10 border-primary/40" : "bg-card/30 border-border/20 hover:border-border/50"
                            }`}>
              ...
              <div className="pt-4 space-y-3">
                <button onClick={handleContinue} className="group block w-full text-center py-4 rounded-full bg-primary text-primary-foreground font-bold text-sm uppercase tracking-widest glow-red hover:scale-[1.02] transition-all duration-300">
                  <span className="flex items-center justify-center gap-2">
                    Continue to Quote <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                  </span>
                </button>
                <p className="text-center text-[11px] text-muted-foreground">Review your build & get a personalized quote</p>
              </div>
            </motion.div>
          </div>
        </div>
